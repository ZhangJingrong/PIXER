...
def g_kernel(width = KERNEL_W, sigma = 1.0):
    res = cv2.getGaussianKernel(width,sigma)
    res=1-(res-numpy.max(res))/(-1*numpy.ptp(res));
    res=res.reshape(width);
    print(res);
    x = numpy.float32(res);
    print(x.shape);
    return x;

def one_kernel(width = KERNEL_W, sigma = 1.0):
    kx = numpy.ones(width);
    kx=kx.reshape(width);
    x = numpy.float32(kx)
    print(x.shape);
...
def gaussian_kernel(width = KERNEL_W, sigma = 4.0):
    ...
    filterx /= filterx.sum()
    res=filterx;
    f=1-(res-numpy.max(res))/(-1*numpy.ptp(res));
    print(filterx.shape);
    return filterx
...
def convolution_cuda(sourceImage,  filterx,  filtery):
    ...
    print('cuda:', grid_rows,grid_cols,block_rows, block_cols);
...
def testImg():
    fname='/home/ict/pickyEye/KLH2/heat/01nov26b.009.007.001.002.png';
    img = cv2.imread(fname,0);
    img=numpy.float32(img)

    print(KERNEL_RADIUS,KERNEL_W)
    filterx = gaussian_kernel() #width = KERNEL_W, sigma = 4.0
    #filterx=one_kernel();
    #filterx=g_kernel();
    destImage = img.copy()
    destImage[:] = numpy.nan
    destImage = convolution_cuda(img,  filterx,  filterx)
    res=destImage
    res=255*(1-(res-numpy.max(res))/(-1*numpy.ptp(res)));

    cv2.imwrite('des.png', res);
